...
class XLStoCSVParser extends Spreadsheet_Excel_Reader
{
                ...
                foreach($data['cells'] as $row) 
                {
                   foreach($row as $cell)
                   {
                       ...
                       if(array_search($cell, $row) == 3){
                            if(strtolower($cell)== 'ãðóçîâûå øèíû'){
                                $tiresvariable = 'ãðóçîâîé';
                                $seasonvariable = null;
                            }
                            elseif (strtolower($cell)== 'ëåãêîãðóçîâûå øèíû'){
                                $tiresvariable = 'ëåãêîâîé';
                                $seasonvariable = null;
                            }
                            else{
                                $tiresvariable = null;
    ...
    function formatSeasons($str) //returns number of '/' in string
    {
        switch (strtolower($str)) {
            case "ëåòî-âñåñåçîíêà":
              return "ëåòíÿÿ-âñåñåçîííàÿ";
              break;
            case "çèìà":
              return "çèìíÿÿ";
              break;
